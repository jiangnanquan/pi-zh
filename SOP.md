...
- **改「模型看到的文本」**（如 F1）：直接用 pi 自带 jiti 加载补丁后的模块，构造样例 state/op 逐条断言。
  至少覆盖三条路径：结算动作 + 有残留（应触发）、结算动作 + 无残留（不应触发）、非结算动作（不应触发）。
- **改「面板渲染行为」**（如 F2）：跑现成探针，它注入任务状态并 mock UI 上下文，断言触发 / 不触发 / 原版对照三条路径：
  ```bash
  node scripts/probe_todo_overlay_cleanup.mjs
  ```
...
### 场景 5：排查「面板结算残留在屏幕上」（F2 生效判定）

现象：全部任务已完成，面板仍以暗色 `○ Todos (n/n)` 滞留，直到下一次 todo 工具调用才消失。

1. `--status` 确认 F2 补丁在位；2. 确认会话已 `/reload`（源码级补丁必须重载才生效）；
3. 行为判定：`node scripts/probe_todo_overlay_cleanup.mjs` —— 三条路径全绿即补丁生效；
4. 若补丁在位仍复现：先用一次 todo 工具调用兜底清理，并把复现现场（折叠态？`/reload` 后？）记入 `00_状态.md` 待查——那属于本补丁覆盖范围之外的成因。
