...
1. **单一职责**：本项目主体是 CLI/TUI 界面与命令注释汉化；另有**维护线 F**（插件功能补丁）承担经用户逐条授权的第三方插件行为修复，边界见原则 11。无论哪条线，都不得触碰模型请求 Payload 的构造、上下文管理、RPC/CBOR 协议，也不得改 pi 本体逻辑。
...
11. **插件功能补丁（维护线 F）**：当第三方插件存在影响协作可靠性的行为缺陷、且插件无可用的扩展点（事件/配置/运行时覆盖）时，允许对其源码打**最小功能补丁**，但必须同时满足五条：① 用户逐条明确授权并记录 `authorized_on`；② 写明 `reason`（现象 + 依据 + 回退分支）；③ 补丁形态限于「新增函数 + 最小调用点替换」，**不改原函数的既有分支语义**，无触发条件时恒等返回原行为；④ 复用 C 线同一套引擎（`--dict i18n/plugin-logic.json`，预检 / `.zh-backup` / jiti 体检 / `--restore`）；⑤ `source_version` 随上游同步维护，失配时严格模式拒绝写盘。F 线补丁只允许作用于**被授权的那一个插件文件**，不得借此改写 pi 本体或其它插件。
...
| **F 线越权改插件** | 未经用户授权就给插件加功能；或补丁改写原函数既有分支语义、拿掉恒等回退（无触发条件时也必须原样返回） | 走维护线 F：`i18n/plugin-logic.json` + `id` / `reason` / `authorized_on` + 恒等回退 + `--restore`；上游改段后只允许重新定位锚点，不得顺手改逻辑 |
