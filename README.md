...
[![Pi Version](https://img.shields.io/badge/Pi-0.87.0-green.svg)](#)
...
- **版本严格跟随**：`pi --version` 严格显示上游官方版本号（当前基准 v0.87.0），不自造版本号。
...
同一条维护线还含**三条用户授权行为补丁**（C 线 1 条 + F 线 2 条，均带 `id` / `reason` / `authorized_on` 与回退分支）：

1. `welcome-header-eager-shell` —— 启动欢迎页先用空数据挂 header 立即上屏，取数完成后再换 header 重绘。
2. `todo-settle-nudge`（**F 线首条**，字典 `i18n/plugin-logic.json`）—— `@juicesharp/rpiv-todo` 的
...
3. `todo-overlay-cleanup`（**F 线第二条**，字典 `i18n/plugin-logic.json`）—— `@juicesharp/rpiv-todo` 的
...
> **已退役的 C 线补丁**（2026-09-22，powerline 0.17.2）：`editor-chrome-thinking-border` 与
> `dock-trim-primary-into-footer` —— 上游已原生实现同类改进（#221 边框跟随思考色、#217 次级详情复用 footer
> 预留行）。退役前先跑两个探针确认未打补丁已达标（紫 6 / 灰 0、末行即状态行），再删除对应 `code_patches` 段。
...
│   └── files/                   # 随包分发的实体文件（pi-cc-extensions.json + 4 个自研扩展）
└── tests/
...
| 组件 | 版本 | 说明 |
| :--- | :--- | :--- |
| `@earendil-works/pi-coding-agent` | `v0.87.0` | 当前严格适配版本 |
